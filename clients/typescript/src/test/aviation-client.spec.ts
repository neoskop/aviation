...
const endpointTestUrl = process.env['ENDPOINT_TEST_URL'] || 'http://localhost:8080';

let changeFeatureTitle = function (sut: AviationClient, newTitle: string): Promise<request.Response> {
    return sut.feature('test-feature-1').catch(err => console.log(err)).then((feature: AviationFeature) => {
...
describe('Aviation client', () => {
    it('should fail to return a feature for wrong endpoints', () => {
        let sut: AviationClient = new AviationClient('http://localhost:1234', 'wrongtoken', 3600, {}, false);
        return sut.feature('test').should.eventually.be.rejected;
    });

    it('should fail to return a feature for wrong token', () => {
        let sut: AviationClient = new AviationClient(endpointTestUrl, 'wrongtoken', 3600, {}, false);
        return sut.feature('test').should.eventually.be.rejected;
    });

    it('should fail to return a feature for non-managed feature', () => {
        let sut: AviationClient = new AviationClient(endpointTestUrl, 'sup3rs3cr3t', 3600, {}, false);
        return sut.feature('test').should.eventually.be.rejected;
    });
    ...
    it('should return feature that can be evaluated', () => {
        let sut: AviationClient = new AviationClient(endpointTestUrl, 'sup3rs3cr3t', 3600, {}, false);
        sut.feature('test-feature-1').catch(err => console.log(err));
        return sut.feature('test-feature-1').should.eventually.be.fulfilled.and.then((f: AviationFeature) => {
    ...
    it('should pickup feature changes immediately via WebSockets', async() => {
        let sut: AviationClient = new AviationClient(endpointTestUrl, 'sup3rs3cr3t');
